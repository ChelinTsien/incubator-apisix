...
local schema = {
    ...
    properties = {
        ...
        service_name = {type = "string", minLength = 0, default = "apisix"},
...
function _M.rewrite(conf, ctx)
    ...
    if not amzdate or string.len(amzdate) < 10  then
        return 401, {message = "header 'x-amz-date' is needed"}
    end
    
    if not authorization or string.len(authorization) < 10 then
        return 401, {message = "invalid authorization"}
    end
    ...
    if not consumers[access_key] or not consumers[access_key].auth_conf then
        return 401, {message = "Invalid access_key"}
    end

    local auth_conf = consumers[access_key].auth_conf
    if auth_table[1] == nil or auth_table[1] ~= string.upper(auth_conf.version) .. "-HMAC-SHA256" then
        return 401, {message = "invalid sign method"}
    end

    if diff_time > auth_conf.expire_time or diff_time < -3600  then
        return 401, {message = "signature expireed"}
    end

    if access_key ~= auth_conf.access_key then
        return 401, {message = "invalid access_key"}
    end
    ...
    if region ~= auth_conf.region then
        return 401, {message = "invalid region"}
    end

    if service_name ~= string.lower(auth_conf.service_name) then
        return 401, {message = "invalid service name"}
    end

    if request_name ~= string.lower(auth_conf.version .. "_request") then
        return 401, {message = "invalid sign  version"}
    end
    ...
    local string_to_sign = string.upper(auth_conf.version) .. "-HMAC-SHA256" .. "\n" .. amzdate .. "\n" .. credential_scope .. "\n"
          .. canonical_request_str

    local signing_key = get_signature_key(auth_conf.secret_key, date_stamp, region, service_name, auth_conf.version)
